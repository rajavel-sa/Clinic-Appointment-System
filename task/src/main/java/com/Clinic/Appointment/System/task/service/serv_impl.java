...
public class serv_impl implements clinic_serv{
    ...
    public String bookAppointment(String doctorType, String patientName, int time) {
        ...
        Optional<appointment_mod> existingAppointmentOpt = appointmentRepo.findAll().stream().filter(app -> app.getP_name().equalsIgnoreCase(patientName)).findFirst();
        ...
        if (existingAppointmentOpt.isPresent()) {
            ...
            if (existingAppointment.getD_spl().equalsIgnoreCase(doctorType)) {
                return "x Patient " + patientName + " already has an appointment with Dr. "+ existingAppointment.getD_name() + " at time slot " + existingAppointment.getTime() + ".";
            } else {
                return "x Patient " + patientName + " cannot book with Dr. " + doctor.getD_name() +" (" + doctorType + ") because they already have an appointment with Dr. " +existingAppointment.getD_name() + " (" + existingAppointment.getD_spl() + ") at " +"time slot " + existingAppointment.getTime() + ".";
            }
        }


        boolean isAlreadyBooked = appointmentRepo.findAll().stream().anyMatch(app -> app.getD_spl().equalsIgnoreCase(doctorType)&& app.getTime() == time);

        if (isAlreadyBooked) {
            List<Integer> bookedTimes = appointmentRepo.findAll().stream().filter(app -> app.getD_spl().equalsIgnoreCase(doctorType)).map(appointment_mod::getTime).collect(Collectors.toList());
    ...
    public String rescheduleAppointment(String doctorType, String patientName, int time) {
        Optional<appointment_mod> existing = appointmentRepo.findAll().stream().filter(app -> app.getP_name().equalsIgnoreCase(patientName) && app.getD_spl().equalsIgnoreCase(doctorType)).findFirst();
        
        if (existing.isEmpty()) {
            return " x No appointment found to reschedule for patient: " + patientName;
        }
        
        boolean isTimeTaken = appointmentRepo.findAll().stream().anyMatch(app -> app.getD_spl().equalsIgnoreCase(doctorType)&& app.getTime() == time);
    ...
    public String cancelAppointment(String patientName) {
        List<appointment_mod> appointments = appointmentRepo.findAll().stream().filter(app -> app.getP_name().equalsIgnoreCase(patientName)).toList();
    ...
    public List<Integer> getAvailableSlots(String doctorType) {
        ...
        List<Integer> bookedTimes = bookedAppointments.stream().filter(appointment -> appointment.getD_spl().equalsIgnoreCase(doctorType)).map(appointment_mod::getTime).toList();
        List<Integer> availableSlots = new ArrayList<>(validSlots);
        availableSlots.removeAll(bookedTimes);
    ...
    public List<appointment_mod> getDoctorSchedule(String doctorType) {
        return appointmentRepo.findAll().stream().filter(app -> app.getD_spl().equalsIgnoreCase(doctorType)).collect(Collectors.toList());
    }
}
